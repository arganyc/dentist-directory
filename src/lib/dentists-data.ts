...
function loadIndex(): DataIndex {
  ...
  // Prefer the full local dataset (gitignored, generated by `npm run import-dentists`);
  // fall back to the committed subset shipped with production deployments.
  const candidates = [
    path.resolve(root, "dentists-data.json"),
    path.resolve(root, "data", "dentists-subset.json"),
  ];
  const filePath = candidates.find((p) => fs.existsSync(p));
  if (!filePath) {
    throw new Error(
      `No dentist dataset found. Looked in:\n  ${candidates.join("\n  ")}\n` +
        `Run 'npm run import-dentists' (full) or 'npm run build:subset' (production subset).`
    );
  }
