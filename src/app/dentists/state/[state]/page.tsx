...
export async function generateMetadata(props: PageProps<"/dentists/state/[state]">) {
  const params = await props.params;
  const stateParam = params.state ?? "";
  const code = codeFromParam(stateParam) ?? stateParam.toUpperCase();
  const stateName = stateNameByCode(code);
  return {
    title: `Find a Dentist in ${stateName} | USDentistsDirectory`,
  };
}

export default async function StateDentistsPage(props: PageProps<"/dentists/state/[state]">) {
  const params = await props.params;
        ...
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          <nav className="text-sm text-slate-500">
            <Link href="/" className="hover:text-blue-700">
              Home
            </Link>{" "}/ <span className="text-slate-700">Find a Dentist</span>
          </nav>
          <h1 className="mt-2 text-3xl font-bold text-slate-900 sm:text-4xl">
            Dentists in {stateName}
          </h1>
          <p className="mt-2 text-slate-600">
            {total === 0 ? (
              <>No dentists found in {stateName}</>
            ) : (
              <>
                ...
                <span className="font-semibold text-slate-900">{total.toLocaleString()}</span> dentists in {stateName}
              </>
            )}
          </p>
        </div>
        ...
        <main>
          {results.length === 0 ? (
            <div className="rounded-xl border border-dashed border-blue-200 bg-white p-12 text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">Try a different state or remove filters.</p>
              <Link
                href="/dentists"
